...
| 80 | `80_store_grant_is_real.sql` | Finishes 61. The purchase ledger, anomaly events, repair returns and upload history were still on `plant_in_scope`, so a **store** tick granted nothing there — at Rehla every row carries one anchor factory, so a keeper granted the shared store saw an empty screen. Adds `store_row_in_scope(store, plant)` and moves those policies onto it. **Deploy the frontend with it** (`storeQuery` replaces `scopeQuery` on the same tables). Strictly widening; `fixed_assets` still factory-only | 🟡 |
