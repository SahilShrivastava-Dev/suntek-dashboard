...
import { MAINT } from '../../../data/mockData';
...
export function Maintenance() {
  ...
  const list = filter === 'all' ? MAINT : MAINT.filter(m => m.type === filter);
    ...
    <>
      ...
      <div className="grid grid-cols-12 gap-5 mb-5">
        ...
        <div className="col-span-12 lg:col-span-3 card p-5">
          ...
          <div className="text-[28px] font-extrabold mt-1 num text-amber-600">5</div>
          <div className="text-[11px] text-amber-600 mt-1">2 pending &gt; 7 days</div>
        </div>
        <div className="col-span-12 lg:col-span-3 card p-5">
          <div className="text-[11px] text-slate-500 uppercase tracking-wider">Scrap items</div>
          <div className="text-[28px] font-extrabold mt-1 num text-red-600">3</div>
          <div className="text-[11px] text-red-600 mt-1">flagged for disposal</div>
        </div>
        <div className="col-span-12 lg:col-span-3 card p-5">
          <div className="text-[11px] text-slate-500 uppercase tracking-wider">Closed this week</div>
          <div className="text-[28px] font-extrabold mt-1 num">18</div>
          <div className="text-[11px] text-green-600 mt-1">↑ 12% vs last</div>
        </div>
      </div>
              ...
              {list.map((m, i) => {
                ...
                return (
                  <tr key={i} style={{ cursor: 'pointer' }}>
                    ...
                    <td>{m.plant}</td>
                    <td className="font-semibold">{m.eq}</td>
                    ...
                    <td className="text-slate-500">{m.by}</td>
                    <td><PicBadge has={m.pic} /></td>
                  </tr>
                );
              })}
            </tbody>
